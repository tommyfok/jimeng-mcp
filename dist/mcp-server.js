...
export class JimengMCPServer {
     ...
     * 检查是否为图像生成相关的操作
     */
    isImageGenerationOperation(name) {
    ...
    setupTools() {
        ...
        // 生成图像并等待完成工具
        this.server.registerTool('generate_image_and_wait', {
            title: '生成图像并等待',
        ...
        }, async ({ prompt, use_pre_llm, seed, width, height, return_url, logo_info, max_wait_time, }) => {
            return await this.withConcurrencyControl(async () => {
                const request = {
                    prompt,
                    use_pre_llm,
                ...
                const response = await this.api.generateImageAndWait(request, config, maxWaitTime);
        ...
        // 获取推荐尺寸工具
        this.server.registerTool('get_recommended_sizes', {
            title: '获取推荐尺寸',
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `推荐图像尺寸配置:\n\n标清1K:\n${Object.entries(sizes.STANDARD_1K)
                            .map(([ratio, size]) => `  ${ratio}: ${size.width} x ${size.height}`)
        ...
        // 获取水印选项工具
        this.server.registerTool('get_watermark_options', {
            title: '获取水印选项',
        ...
        // 验证图像尺寸工具
        this.server.registerTool('validate_image_size', {
            title: '验证图像尺寸',
        ...
        // 验证提示词工具
        this.server.registerTool('validate_prompt', {
            title: '验证提示词',
    ...
    setupResources() {
        ...
        this.server.registerResource('task-status', new ResourceTemplate('task://{taskId}/status', { list: undefined }), {
            ...
            try {
                const response = await this.api.queryTask(taskId);
                const statusInfo = {
                    task_id: taskId,
