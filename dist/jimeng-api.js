...
export class JimengAPI {
     ...
     * @returns 任务提交响应
     */
    async generateImageToImage(request) {
        try {
            // 验证输入参数
     ...
     * @returns 任务查询响应
     */
    async queryTask(taskId, reqKey = JIMENG_API_CONSTANTS.REQ_KEY_T2I, config) {
        try {
            const payload = {
                req_key: reqKey,
                task_id: taskId,
     ...
     * @returns 是否有效
     */
    validateImageToImageSize(width, height) {
        // 检查尺寸范围
     ...
     * @param request 图生图请求参数
     */
    validateImageToImageRequest(request) {
        // 验证提示词
        ...
        if (request.width && request.height) {
            if (!this.validateImageToImageSize(request.width, request.height)) {
                throw new Error('图像尺寸无效，宽度和高度必须在512到2016之间，且宽高比在1:3到3:1之间');
            }
        }
    }
    /**
     ...
     * @returns base64编码字符串
     */
    async imageFileToBase64(filePath) {
        try {
