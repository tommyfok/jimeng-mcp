...
export class JimengMCPServer {
  ...
  private lastRequestTime: number = 0; // 上次请求时间
  private readonly REQUEST_COOLDOWN = 2000; // 请求冷却时间（毫秒）
  ...
  private setupTools() {
    ...
    this.server.registerTool(
      'generate_image',
      {
        title: '即梦图像生成',
        description:
          '使用即梦AI 4.0生成图像。支持文生图、图生图、多图参考生成。支持设置尺寸、比例、编辑强度等。',
        inputSchema: {
          ...
          if (size === undefined && width === undefined && height === undefined) {
            ...
            if (width !== undefined && height !== undefined) {
              ...
              if (area < minArea || area > maxArea) {
                throw new Error(
                  `图片面积 (width * height) 必须在 1024*1024 到 4096*4096 之间，` +
      ...
      'query_task',
      {
        ...
        inputSchema: {
          ...
          req_key: z
            ...
            .optional()
            .describe(
              '服务标识，默认使用 jimeng_t2i_v40（4.0版本文生图和图生图统一使用此标识）'
            ),
